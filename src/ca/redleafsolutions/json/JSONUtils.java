...
public class JSONUtils {
	...
	/*
	public static JSONObject item2object (JSONItem item) throws JSONValidationException {
		Trace.info ("This method should be removed when all JSONObjects are converted to JSONItems (" + (++refcounter) + ")");
	//		if (!item.isObject ())
	//			throw new JSONValidationException ("Array can not be converted to object");
	//		for (Object okey:item.listKeys ()) {
	//			String key = okey.toString ();
	//			Object o = item.get (key);
	//			if (o instanceof JSONItem.Object) {
	//				item.put (key, item2object ((JSONItem.Object)o));
	//			} else if (o instanceof JSONItem.Array) {
	//				item.put (key, item2array ((JSONItem.Array)o));
	//			} else if (o instanceof JSONWritable) {
	//				item.put (key, ((JSONWritable)o).toJSON ());
	//			} else if (o instanceof JSONWritable2) {
	//				JSONItem json = ((JSONWritable2)o).toJSON ();
	//				if (json.isArray ()) {
	//					item.put (key, item2array (json));
	//				} else {
	//					item.put (key, item2object (json));
	//				}
	//			}
	//		}
		return ((JSONItem.Object)item).json;
	}
	
	public static JSONArray item2array (JSONItem item) throws JSONValidationException {
		Trace.info ("This method should be removed when all JSONObjects are converted to JSONItems (" + (++refcounter) + ")");
	//		if (!item.isArray ())
	//			throw new JSONValidationException ("Object can not be converted to array");
	//		for (Object okey:item.listKeys ()) {
	//			String key = okey.toString ();
	//			Object o = item.get (key);
	//			if (o instanceof JSONItem.Object) {
	...
	//			}
	//		}
		return ((JSONItem.Array)item).json;
	}
	
	public static JSONItem json2item (JSONObject json) throws JSONValidationException {
		Trace.info ("This method should be removed when all JSONObjects are converted to JSONItems (" + (++refcounter) + ")");
	...
	//					json.put (key, json2item ((JSONObject)o));
	//				} else if (o instanceof JSONArray) {
	//					json.put (key, json2item ((JSONArray)o));
	//				}
	...
	public static JSONItem json2item (JSONArray json) throws JSONValidationException {
		Trace.info ("This method should be removed when all JSONObjects are converted to JSONItems (" + (++refcounter) + ")");
	//		for (int i=0; i<json.length (); ++i) {
	//			try {
	//				Object o = json.get (i);
	//				if (o instanceof JSONObject) {
	//					json.put (i, json2item ((JSONObject)o));
	//				} else if (o instanceof JSONArray) {
	//					json.put (i, json2item ((JSONArray)o));
	//				}
	//			} catch (JSONException e) {
	//			}
	//		}
	...
	public static JSONObject toJSONObject (Object o) throws JSONValidationException {
		return (JSONObject)toJSON (o);
	}
	
	public static JSONArray toJSONArray (Object o) throws JSONValidationException {
				...
				if (v1 instanceof JSONObject) {
					...
					for (Iterator<String> it = j1.keys (); it.hasNext ();) {
						...
						try {
							...
							// NodeDiff ndiff = new NodeDiff (key1);
							// ndiff.only1 (jv1);
							// nodediff.add (ndiff);
							// nodediff.only1 (jv1);
						}
						nodediff.add (compareValues (key1, jv1, jv2));
					}

					for (Iterator<String> it = j2.keys (); it.hasNext ();) {
						String key2 = it.next ();
						...
						try {
							...
							if (jv1 == null) {
								// nodediff.only2 (jv2);
								nodediff.add (compareValues (key2, jv1, jv2));
							} else {
								// this case was already taken care of in the
								// first loop
							}
					...
					JSONArray arr1 = (JSONArray)v1;
					JSONArray arr2 = (JSONArray)v2;
					...
					while (arr1.length () > 0) {
						...
						try {
							...
							throw new JSONValidationException ("0");
						}
						boolean found = false;
						for (int i = 0; !found && i < arr2.length (); ++i) {
							Object jv2;
							try {
								jv2 = arr2.get (i);
							} catch (JSONException e) {
								throw new JSONValidationException ("" + i);
							}
							NodeDiff cvarr = compareValues (null, jv1, jv2);
							if (cvarr.isEmpty ()) {
								arr2.remove (i);
								found = true;
							}
